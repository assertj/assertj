...
public class Arrays {
  ...
  /** 
   * Returns an T[] from the {@link AtomicTArray}, null if the given atomic array is null.
   * 
   * @param atomicReferenceArray the {@link AtomicTArray} to convert to T[].
   * @return an T[].
   */
  ...
  public static <T> T[] array(AtomicReferenceArray<T> atomicReferenceArray) {
    ...
    return list.toArray((T[]) Array.newInstance(atomicReferenceArray.get(0).getClass(), length));
  }
