 ...
 */
public class ShouldHaveLinesCount extends BasicErrorMessageFactory {

  /**
   * Creates a new </code>{@link ShouldHaveLinesCount}</code>.
  ...
  public static ErrorMessageFactory shouldHaveLinesCount(Object actual, int actualSize, int expectedSize) {
    return new ShouldHaveLinesCount(actual, actualSize, expectedSize);
  }

  private ShouldHaveLinesCount(Object actual, int actualSize, int expectedSize) {
    ...
    super(format("\nExpected lines count:<%s> but was:<%s> in:\n<%s>", expectedSize, actualSize, "%s"), actual);
  }
}
