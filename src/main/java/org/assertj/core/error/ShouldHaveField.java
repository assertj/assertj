...
public class ShouldHaveField extends BasicErrorMessageFactory {
  ...
  /**
   * Creates a new </code>{@link ShouldHaveField}</code>.
   *
   * @param actual the actual value in the failed assertion.
   * @param name expected name of field for this class
   * @return the created {@code ErrorMessageFactory}.
   */
  public static ErrorMessageFactory shouldHaveField(Object actual, String name) {
    return new ShouldHaveField(actual, name);
  }

  private ShouldHaveField(Object actual, String name) {
    super("%nExpecting%n  <%s>%nto have "
          + "field:%n  <%s>", actual, name);
  }
}
