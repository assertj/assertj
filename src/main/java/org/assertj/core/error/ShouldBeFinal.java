...
/**
 * Creates an error message indicating that an assertion that verifies that a class is (or not) final.
 *
...
public class ShouldBeFinal extends BasicErrorMessageFactory {
  ...
  private ShouldBeFinal(Class<?> actual, boolean positive) {
    super("%nExpecting%n  <%s>%nto " + (positive ? "" : " not ") + "be final class", actual);
  }
