...
public interface FloatingPointNumberAssert<S extends  FloatingPointNumberAssert<S, A>, A extends Number> extends NumberAssert<S, A> {
   ...
   * Example with double:
   * <pre><code class='java'> // assertions will pass:
   * assertThat(8.3).isNotCloseTo(new Double(8.0), within(0.2));
   *
   * // you can use offset if you prefer
   * assertThat(8.3).isNotCloseTo(new Double(8.0), offset(0.2));
   *
   * // assertions will fail
   * assertThat(8.1).isNotCloseTo(new Double(8.0), within(0.1));
   * assertThat(8.1).isNotCloseTo(new Double(8.0), within(0.2));</code></pre>
   *
   * @param expected the given number to compare the actual value to.
   * @param offset the given positive offset.
   * @return {@code this} assertion object.
