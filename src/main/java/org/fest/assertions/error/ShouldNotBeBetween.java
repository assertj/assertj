...
/**
 * Creates an error message indicating that an assertion that verifies that a {@link Date} is not between start - end dates (inclusive or not) failed.
 *
...
public class ShouldNotBeBetween extends BasicErrorMessageFactory {
  ...
  private ShouldNotBeBetween(Date actual, Date start, Date end, boolean inclusiveStart,boolean inclusiveEnd) {
    super("expected:<%s> not to be in period %s%s, %s%s", actual, inclusiveStart ? "[" : "]", start, inclusiveEnd ? "]" : "[", end);
  }
}
