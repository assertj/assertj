...
/**
 * Creates an error message indicating that an assertion that verifies that a {@link Date} is between start - end dates (inclusive or not) failed.
 *
...
public class ShouldBeBetween extends BasicErrorMessageFactory {
  ...
  public static ErrorMessageFactory shouldBeBetween(Date actual, Date start, Date end, boolean inclusiveStart,boolean inclusiveEnd) {
    return new ShouldBeBetween(actual, start, end, inclusiveStart, inclusiveEnd);
  }

  private ShouldBeBetween(Date actual, Date start, Date end, boolean inclusiveStart,boolean inclusiveEnd) {
    super("expected:<%s> to be in period %s%s, %s%s", actual, inclusiveStart ? '[' : ']', start, end, inclusiveEnd ? ']' : '[');
  }
}
