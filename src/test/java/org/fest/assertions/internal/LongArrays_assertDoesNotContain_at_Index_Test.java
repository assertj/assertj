...
public class LongArrays_assertDoesNotContain_at_Index_Test {
  ...
  @Test public void should_fail_if_actual_is_null() {
    ...
    arrays.assertDoesNotContain(info, null, 8L, atIndex(0));
  }

  @Test public void should_pass_if_actual_is_empty() {
    arrays.assertDoesNotContain(info, emptyArray(), 8L, atIndex(0));
  }

  @Test public void should_throw_error_if_Index_is_null() {
    thrown.expectNullPointerException("Index should not be null");
    arrays.assertDoesNotContain(info, actual, 8L, null);
  }

  @Test public void should_pass_if_Index_is_out_of_bounds() {
    arrays.assertDoesNotContain(info, actual, 8L, atIndex(6));
  }

  @Test public void should_fail_if_actual_contains_value_at_index() {
    try {
      arrays.assertDoesNotContain(info, actual, 6L, atIndex(0));
      fail();
    } catch (AssertionError e) {}
    verify(failures).failure(info, containsAtIndex(wrap(actual), 6L, atIndex(0)));
  }
}
