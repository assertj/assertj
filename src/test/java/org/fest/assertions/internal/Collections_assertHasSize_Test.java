 ...
 */
public class Collections_assertHasSize_Test extends AbstractTest_for_Collections {
  ...
  public void should_pass_if_size_of_actual_is_equal_to_expected_size_whatever_custom_comparison_strategy_is() {
    collectionsWithCaseInsensitiveComparisonStrategy.assertHasSize(someInfo(), list("Luke", "Yoda"), 2);
  }
  
  @Test
  public void should_fail_if_actual_is_null_whatever_custom_comparison_strategy_is() {
    thrown.expectAssertionError(actualIsNull());
    collectionsWithCaseInsensitiveComparisonStrategy.assertHasSize(someInfo(), null, 8);
  }
  ...
  public void should_fail_if_size_of_actual_is_not_equal_to_expected_size_whatever_custom_comparison_strategy_is() {
    ...
    Collection<String> actual = list("Yoda");
    try {
      collectionsWithCaseInsensitiveComparisonStrategy.assertHasSize(info, actual, 8);
    } catch (AssertionError e) {
      verify(failures).failure(info, shouldHaveSize(actual, actual.size(), 8));
      return;
    }
