...
public class ExtendedByTypeComparator_compareTo_Test {

  private static final Map<Class<?>, Comparator<?>> COMPARATORS_BY_TYPE = new HashMap<>();

  private static final ExtendedByTypeComparator EXTENDED_STANDARD_COMPARATOR = new ExtendedByTypeComparator(COMPARATORS_BY_TYPE);
  private static final ExtendedByTypeComparator EXTENDED_FIELD_BY_FIELD_COMPARATOR =
    new ExtendedByTypeComparator(new FieldByFieldComparator(), COMPARATORS_BY_TYPE);

  @Parameter
  public ExtendedByTypeComparator extendedComparator;

  @Parameters
  public static Iterable<ExtendedByTypeComparator> data() {
  ...
  public void should_return_equal_if_objects_are_equal_by_default_comparator() {
    assertThat(EXTENDED_STANDARD_COMPARATOR.compare(new Jedi("Yoda", "Green"), new Jedi("Yoda", "Any"))).isZero();
