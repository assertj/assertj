...
/**
 * Tests for <code>{@link Float2DArrays#assertHasSize(AssertionInfo, float[][], int, int)}</code>.
 * 
 * @author Alex Ruiz
 * @author Joel Costigliola
 * @author Maciej Wajcht
 */
public class Float2DArrays_assertHasSize_Test extends Float2DArraysBaseTest {
  ...
  public void should_fail_if_second_dimension_size_of_actual_is_not_equal_to_expected_size() {
    assertThatExceptionOfType(AssertionError.class).isThrownBy(() -> arrays.assertHasSize(someInfo(), actual, 2, 10))
      .withMessage(shouldHaveSize(new float[] {0.0f, 2.0f, 4.0f}, 3, 10, 0).create());
  }
