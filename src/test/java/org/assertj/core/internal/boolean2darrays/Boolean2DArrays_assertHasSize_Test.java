...
/**
 * Tests for <code>{@link Boolean2DArrays#assertHasSize(AssertionInfo, boolean[][], int, int)}</code>.
 * 
 * @author Alex Ruiz
 * @author Joel Costigliola
 * @author Maciej Wajcht
 */
public class Boolean2DArrays_assertHasSize_Test extends Boolean2DArraysBaseTest {
  ...
  public void should_fail_if_second_dimension_size_of_actual_is_not_equal_to_expected_size() {
    assertThatExceptionOfType(AssertionError.class).isThrownBy(() -> arrays.assertHasSize(someInfo(), actual, 2, 10))
      .withMessage(shouldHaveSize(new boolean[] {true, false, false}, 3, 10, 0).create());
  }
