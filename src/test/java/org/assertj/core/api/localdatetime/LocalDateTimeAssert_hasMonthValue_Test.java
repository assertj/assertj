...
class LocalDateTimeAssert_hasMonthValue_Test {
  ...
  void should_fail_if_actual_has_not_same_month_value() {
    ...
    int monthVal = 10;
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> assertThat(actual).hasMonthValue(monthVal));
    // THEN
    then(assertionError).hasMessage(shouldHaveDateField(actual, "month", monthVal).create());
  }
