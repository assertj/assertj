...
public class RecursiveAssertionDriver_CollectionHandlingTest extends AbstractRecursiveAssertionDriverTestBase {
  ...
  void should_assert_over_collection_object_but_not_elements_when_policy_is_collection_only() {
    ...
    RecursiveAssertionConfiguration configuration = RecursiveAssertionConfiguration.builder()
      .withCollectionAssertionPolicy(RecursiveAssertionConfiguration.CollectionAssertionPolicy.COLLECTION_OBJECT_ONLY)
      .build();
    ...
    // THEN
    BDDAssertions.then(failedFields).hasSize(2).contains(FieldLocation.rootFieldLocation(),
                                                         FieldLocation.rootFieldLocation().field("collection"));
  }
  ...
  void should_assert_over_array_object_but_not_elements_when_policy_is_collection_only() {
    ...
    // THEN
    BDDAssertions.then(failedFields).hasSize(2).contains(FieldLocation.rootFieldLocation(),
                                                         FieldLocation.rootFieldLocation().field("array"));
  }
  ...
  void should_assert_over_collection_elements_but_not_collection_when_policy_is_elements_only() {
    // GIVEN
    RecursiveAssertionConfiguration configuration = RecursiveAssertionConfiguration.builder()
      .withCollectionAssertionPolicy(RecursiveAssertionConfiguration.CollectionAssertionPolicy.ELEMENTS_ONLY)
      .build();
    RecursiveAssertionDriver objectUnderTest = new RecursiveAssertionDriver(configuration);
    Object testObject = testObjectWithACollection();
    // WHEN
    List<FieldLocation> failedFields = objectUnderTest.assertOverObjectGraph(failingMockPredicate, testObject);
    // THEN
    BDDAssertions.then(failedFields).hasSize(3).contains(FieldLocation.rootFieldLocation(),
                                                         FieldLocation.rootFieldLocation().field("collection").field("[0]"),
                                                         FieldLocation.rootFieldLocation().field("collection").field("[1]"));
  }
  ...
  void should_assert_over_array_elements_but_not_object_when_policy_is_collection_only() {
    ...
    // THEN
    BDDAssertions.then(failedFields).hasSize(3).contains(FieldLocation.rootFieldLocation(),
                                                         FieldLocation.rootFieldLocation().field("array").field("[0]"),
                                                         FieldLocation.rootFieldLocation().field("array").field("[1]"));
  }
  ...
  void should_assert_over_collection_object_and_elements_when_policy_is_object_and_elements() {
    ...
    List<FieldLocation> failedFields = objectUnderTest.assertOverObjectGraph(failingMockPredicate, testObject);
    // THEN
    BDDAssertions.then(failedFields).hasSize(4).contains(FieldLocation.rootFieldLocation(),
                                                         FieldLocation.rootFieldLocation().field("collection"),
                                                         FieldLocation.rootFieldLocation().field("collection").field("[0]"),
                                                         FieldLocation.rootFieldLocation().field("collection").field("[1]"));
  }
