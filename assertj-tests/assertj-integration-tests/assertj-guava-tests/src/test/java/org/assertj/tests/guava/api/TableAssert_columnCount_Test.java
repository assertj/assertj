...
class TableAssert_columnCount_Test extends TableAssertBaseTest {
  ...
  void should_fail_if_actual_is_null() {
    ...
    actual = null;
    // WHEN
    var assertionError = expectAssertionError(() -> assertThat(actual).columnCount());
    // THEN
  ...
  void should_hand_over_column_count_as_integer_assert_actual() {
    ...
    then(returnedAssertion.info.descriptionText()).contains("checking column count");
  }

  @Test
  void javadoc_example_should_work() {
    assertThat(actual).columnCount()
                      .isGreaterThan(1)
                      .isLessThan(5)
                      .isEqualTo(3);
  }
