...
class SoftAssertionsExtension_PER_METHOD_Concurrency_Test {
  ...
  static class ConcurrencyTest {
    ...
    static void waitForFlag(int flagNum) {
      try {
        if (!flags[flagNum].await(500000, MILLISECONDS)) {
  ...
  @Test
  void concurrent_tests_with_explicit_per_method_annotation_do_not_interfere() {
