...
class WithAssertions_assertThatFile_Test {
  ...
  void should_accept_File() {
    ...
    // WHEN
    AbstractFileAssert<?> result = withAssertions.assertThatFile(actual);
    // THEN
    result.hasName("test");
  }
  ...
  void should_accept_bounded_generic_File_subtype() {
    // WHEN
    AbstractFileAssert<?> result = withAssertions.assertThatFile(getFileSubtype());
    // THEN
    result.isNotNull();
  }
