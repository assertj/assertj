...
class BDDAssertions_thenThrownBy_Test {

  @Test
  void should_work_with_runtime_exception_thrown() {
    // WHEN/THEN
    thenThrownBy(codeThrowing(new IllegalArgumentException("boom"))).isInstanceOf(IllegalArgumentException.class)
                                                                    .hasMessage("boom");
  }
  ...
  void should_work_with_throwable_thrown() {
    // WHEN/THEN
    thenThrownBy(codeThrowing(new Throwable("boom"))).isInstanceOf(Throwable.class)
                                                     .hasMessage("boom");
  }
  ...
  void should_work_with_method_reference_having_vararg_parameter() {
    // WHEN/THEN
    thenThrownBy(BDDAssertions_thenThrownBy_Test::methodThrowing).isInstanceOf(Exception.class)
                                                                 .hasMessage("boom");
  }
  ...
  void should_support_description() {
    ...
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> thenThrownBy(codeThrowing(throwable), "Test %s",
                                                                            "code").hasMessage("bam"));
    // THEN
  ...
  void should_fail_if_no_throwable_was_thrown() {
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> thenThrownBy(() -> {}).hasMessage("boom ?"));
    // THEN
  ...
  void should_fail_with_proper_message_when_assertion_is_failing() {
    ...
    Throwable throwable = new Exception("boom");
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> thenThrownBy(codeThrowing(throwable)).hasMessage("bam"));
    // THEN
