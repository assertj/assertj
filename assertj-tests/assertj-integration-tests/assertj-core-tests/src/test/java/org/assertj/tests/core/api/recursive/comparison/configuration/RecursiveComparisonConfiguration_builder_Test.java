...
class RecursiveComparisonConfiguration_builder_Test {
  ...
  void should_set_ignoreNonExistentFields() {
    ...
    boolean value = RandomUtils.secure().randomBoolean();
    // WHEN
    RecursiveComparisonConfiguration configuration = configBuilder().withIgnoreNonExistenceFields(value).build();
    // THEN
    then(configuration.getIgnoreNonExistentFields()).isEqualTo(value);
  }
  ...
  void should_set_ignoreNonExistentFields_with_shortcut_method() {
    ...
    // THEN
    then(configuration.getIgnoreNonExistentFields()).isFalse();
  }
