...
class WithAssertions_assertThatThrownBy_Test {
  ...
  void should_work_with_runtime_exception_thrown() {
    // WHEN/THEN
    withAssertions.assertThatThrownBy(codeThrowing(new IllegalArgumentException("boom")))
  ...
  void should_work_with_throwable_thrown() {
    // WHEN/THEN
    withAssertions.assertThatThrownBy(codeThrowing(new Throwable("boom"))).isInstanceOf(Throwable.class)
                  .hasMessage("boom");
  }
  ...
  void should_work_with_method_reference_having_vararg_parameter() {
    // WHEN/THEN
    withAssertions.assertThatThrownBy(WithAssertions_assertThatThrownBy_Test::methodThrowing).isInstanceOf(Exception.class)
                  .hasMessage("boom");
  }
  ...
  void should_support_description() {
    ...
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> withAssertions.assertThatThrownBy(codeThrowing(throwable),
                                                                                                 "Test %s",
                                                                                                 "code")
                                                                             .hasMessage("bam"));
    // THEN
  ...
  void should_fail_if_no_throwable_was_thrown() {
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> withAssertions.assertThatThrownBy(() -> {}).hasMessage("boom ?"));
    // THEN
