...
class BDDAssertions_thenIllegalStateException_Test {
  ...
  void should_pass_if_IllegalStateException_is_thrown() {
    ...
    // WHEN/THEN
    thenIllegalStateException().isThrownBy(throwingSupplier).withMessage("boom");
  }
  ...
  void should_fail_if_IllegalStateException_is_not_thrown() {
    ...
    // WHEN
    AssertionError assertionError = expectAssertionError(() -> thenIllegalStateException().isThrownBy(throwingSupplier));
    // THEN
