...
class CompletableFutureAssert_isCompletedWithValueSatisfying_Consumer_Test {
  ...
  private final Consumer<String> passingAssertions = value -> {
    assertThat(value).isNotNull();
    assertThat(value).isEqualTo("done");
  };

  private final Consumer<String> failingAssertions = value -> {
    assertThat(value).isNotNull();
  ...
  void should_fail_when_completable_future_is_null() {
    ...
    // WHEN
    var assertionError = expectAssertionError(() -> assertThat(future).isCompletedWithValueSatisfying(passingAssertions));
    // THEN
  ...
  void should_pass_if_completable_future_is_completed_and_requirements_satisfied() {
    ...
    // THEN
    then(future).isCompletedWithValueSatisfying(passingAssertions);
  }
  ...
  void should_fail_if_requirements_not_satisfied() {
    ...
    CompletableFuture<String> future = completedFuture("done");
    // WHEN
    var assertionError = expectAssertionError(() -> assertThat(future).isCompletedWithValueSatisfying(failingAssertions));
    // THEN
    then(assertionError).hasMessage(shouldBeEqualMessage("\"done\"", "\"fail\""));
  }
