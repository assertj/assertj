...
class CompletableFutureAssert_isCompletedWithValueSatisfying_Condition_Test {
  ...
  private final Condition<String> metCondition = new TestCondition<>(true);
  private final Condition<String> unmetCondition = new TestCondition<>();
  ...
  void should_fail_when_completable_future_is_null() {
    ...
    // WHEN
    var assertionError = expectAssertionError(() -> assertThat(future).isCompletedWithValueSatisfying(metCondition));
    // THEN
  ...
  void should_pass_if_completable_future_is_completed_and_condition_satisfied() {
    ...
    // THEN
    then(future).isCompletedWithValueSatisfying(metCondition);
  }
  ...
  void should_fail_if_condition_not_satisfied() {
    ...
    // WHEN
    var assertionError = expectAssertionError(() -> assertThat(future).isCompletedWithValueSatisfying(unmetCondition));
    // THEN
    then(assertionError).hasMessage(shouldBe("done", unmetCondition).create());
  }
