...
class ComparisonDifference_compareTo_Test {
  ...
  void should_order_differences_by_alphabetical_path_ignoring_dot_separator() {
    ...
    // THEN
    then(differences).extracting(ComparisonDifference::getConcatenatedPath)
                     .containsExactly("aa", "aaa", "a.b", "a.b.c", "a.c", "b");
  }

  private static ComparisonDifference comparisonDifference(String path) {
    return new ComparisonDifference(dualValue(path, RandomStringUtils.secure().next(5), RandomStringUtils.secure().next((6))));
  }
