...
public class SoftAssertionsGenerator {
  ...
  public static void main(String[] args) {
    ...
    // TODO: recursive assertion/comparison: delegate methods (nice to have)
    // TODO: get actual javadoc if absent or inherited from since soft assert classes don't inherit from object assert ones.
    // TODO: use maven generate-sources and avoid formatting
  ...
  private static void generateSoftAssertionFor(Class<? extends Assert> assertClass) {
    ...
    TypeSpec.Builder softAssertTypeBuilder = TypeSpec.classBuilder(softAssertClassName)
                                                     ...
                                                     .addMethod(generateConstructor(actualParameter, assertField, assertClass))
    ...
    // ideally should be before package declaration but that does seem to be possible with javapoet
    int currentYear = ZonedDateTime.now().getYear();
    softAssertTypeBuilder.addJavadoc("""
        Copyright 2012-2026 the original author or authors.
        ...
        See the License for the specific language governing permissions and
        limitations under the License.
        """.replace("2012-2026", "2012-%d".formatted(currentYear)));
    try {
  ...
  private static @NonNull String getJavadoc(Method method) {
    var compilationUnit = getCompilationUnit(getClassDefining(method));
    ...
    for (MethodDeclaration methodDeclaration : methodDeclarations) {
      ...
      javadocText = javadocText.replaceAll("link Date}", "link java.util.Date}");
      ...
      javadocText = javadocText.replaceAll("link Assert}", "link org.assertj.core.api.Assert}");
      javadocText = javadocText.replaceAll("link AssertWithComparator", "link org.assertj.core.api.AssertWithComparator");
      javadocText = javadocText.replaceAll("link AbstractAssert", "link org.assertj.core.api.AbstractAssert");
      javadocText = javadocText.replaceAll("link AbstractObjectAssert", "link org.assertj.core.api.AbstractObjectAssert");
      javadocText = javadocText.replaceAll("link AbstractObjectAssert", "link org.assertj.core.api.AbstractObjectAssert");
      javadocText = javadocText.replaceAll("link AbstractThrowableAssert", "link org.assertj.core.api.AbstractThrowableAssert");
      javadocText = javadocText.replaceAll("link StandardRepresentation",
                                           "link org.assertj.core.presentation.StandardRepresentation");
      javadocText = javadocText.replaceAll("link ComparisonDifference",
                                           "link org.assertj.core.api.recursive.comparison.ComparisonDifference");
      javadocText = javadocText.replaceAll("link DefaultRecursiveAssertionIntrospectionStrategy",
                                           "link org.assertj.core.api.recursive.assertion.DefaultRecursiveAssertionIntrospectionStrategy");
      javadocText = javadocText.replaceAll("@throws IntrospectionError",
                                           "@throws org.assertj.core.util.introspection.IntrospectionError");
      ...
      javadocText = javadocText.replaceAll(Pattern.quote("@param instanceOfAssertFactory"), "@param softAssertFactory");
      ...
      javadocText = javadocText.replaceAll(Pattern.quote("@see InstanceOfAssertFactory"), "@see DefaultSoftAssertFactory");
      javadocText = javadocText.replaceAll(Pattern.quote("@see InstanceOfAssertFactories"), "@see SoftAssertFactories");
      javadocText = javadocText.replaceAll(Pattern.quote("{@link InstanceOfAssertFactories}"), "{@link SoftAssertFactories}");
      ...
      javadocText = javadocText.replaceAll(Pattern.quote("extracting(Function)"), "extracting(java.util.function.Function)");
      javadocText = javadocText.replaceAll(Pattern.quote("$"), "&#36;");
      return javadocText;
    }
    return "";
  }

  private static @NonNull Class<?> getClassDefining(Method method) {
    Class<?> definingClass = method.getDeclaringClass();
    while (definingClass.getSuperclass() != Object.class && definingClass.getSuperclass() != null) {
      try {
        definingClass.getSuperclass().getMethod(method.getName(), method.getParameterTypes());
      } catch (NoSuchMethodException e) {
        break;
      }
      definingClass = definingClass.getSuperclass();
    }
    // found the class defining the method but maybe it came from an interface
    if (definingClass.getInterfaces().length == 0) return definingClass;

    Method methodInDefiningInterface = null;
    for (Class<?> interfaceClass : definingClass.getInterfaces()) {
      try {
        methodInDefiningInterface = interfaceClass.getMethod(method.getName(), method.getParameterTypes());
      } catch (NoSuchMethodException e) {
        // check the next interface
      }
    }
    return methodInDefiningInterface == null ? definingClass : methodInDefiningInterface.getDeclaringClass();
  }
  ...
  private static MethodSpec generateConstructor(ParameterSpec assertParameter, FieldSpec assertField,
                                                Class<? extends Assert> assertClass) {
    ...
    if (assertClass == RecursiveComparisonAssert.class) {
      ...
      assertParameter = ParameterSpec.builder(ParameterizedTypeName.get(ClassName.get(RecursiveComparisonAssert.class),
                                                                        WILDCARD_TYPE),
                                              assertFieldValue)
                                     .build();
    } else if (assertClass == RecursiveAssertionAssert.class) {
    ...
    return MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC)
                     ...
                     .addStatement("this.$N = " + assertFieldValue, assertField)
  ...
  private static MethodSpec.Builder generateUsingRecursiveComparisonMethod(Method navigationMethod, FieldSpec assertField) {
    var statement = "return new SoftRecursiveComparisonAssert<>($N.usingRecursiveComparison(), errorCollector)";
    MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(navigationMethod.getName())
                                                 .addModifiers(Modifier.PUBLIC)
                                                 .returns(ParameterizedTypeName.get(ClassName.get(SoftRecursiveComparisonAssert.class),
                                                                                    WILDCARD_TYPE));
    if (navigationMethod.getParameterCount() == 1) {
      ...
      statement = "return new SoftRecursiveComparisonAssert<>($N.usingRecursiveComparison(" + configParameter.getName()
                  + "), errorCollector)";
    }
  ...
  private static MethodSpec.Builder generateUsingRecursiveAssertionMethod(Method navigationMethod, FieldSpec assertField) {
    ...
    if (navigationMethod.getParameterCount() == 1) {
      Parameter configParameter = navigationMethod.getParameters()[0];
      methodBuilder.addParameter(configParameter.getParameterizedType(), configParameter.getName());
      statement = "return new SoftRecursiveAssertionAssert($N.usingRecursiveAssertion(" + configParameter.getName()
                  + "), errorCollector)";
    }
    return methodBuilder.addStatement(statement, assertField);
  }
  ...
  private static boolean isUsingRecursiveComparison(Method navigationMethod) {
