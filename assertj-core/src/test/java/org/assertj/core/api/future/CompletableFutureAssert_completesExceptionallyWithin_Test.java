...
class CompletableFutureAssert_completesExceptionallyWithin_Test extends AbstractFutureTest {
  ...
  void should_pass_if_future_execution_fails() {
    ...
    // WHEN/THEN
    then(completableFuture).completesExceptionallyWithin(1, MILLISECONDS)
                           .withThrowableOfType(ExecutionException.class)
                           .withMessageContaining("boom!");
    then(completableFuture).completesExceptionallyWithin(Duration.ofMillis(1))
                           .withThrowableThat()
  ...
  void should_fail_when_wait_is_interrupted() {
    // GIVEN
    CompletableFuture<Void> completableFuture = futureCompletingAfter(ONE_SECOND, executorService);
    // WHEN
    Thread.currentThread().interrupt();
    AssertionError assertionError = expectAssertionError(() -> assertThat(completableFuture).completesExceptionallyWithin(2,
                                                                                                                          SECONDS));
    // THEN
