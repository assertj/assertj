...
class ShouldMatchPattern_create_Test {
  ...
  void should_create_error_message() {
    ...
    // THEN
    then(message).isEqualTo(format("[Test] %nExpecting actual:%n  \"Yoda\"%nto match pattern:%n  \"Luke\""));
  }
  ...
  void should_create_error_message_escaping_percent() {
    ...
    String message = factory.create(new TextDescription("Test"), STANDARD_REPRESENTATION);
    // THEN
    then(message).isEqualTo(format("[Test] %nExpecting actual:%n  \"%%%%E\"%nto match pattern:%n  \"fffff\""));
  }
}
