...
class ShouldHaveCompletedExceptionallyWithin_create_Test {
  ...
  void should_create_error_message() {
    // WHEN
    String error = shouldHaveCompletedExceptionallyWithin(new CompletableFuture<>(),
                                                          1, TimeUnit.SECONDS).create(new TestDescription("TEST"));
    // THEN
  ...
  void should_create_error_message_Duration() {
    // WHEN
    String error = shouldHaveCompletedExceptionallyWithin(new CompletableFuture<>(),
                                                          Duration.ofSeconds(1)).create(new TestDescription("TEST"));
    // THEN
    then(error).isEqualTo(format("[TEST] %n" +
                                 "Expecting%n" +
                                 "  <CompletableFuture[Incomplete]>%n" +
