...
class Maps_assertHasEntrySatisfying_with_key_and_condition_Test extends MapsBaseTest {
  ...
  void should_fail_if_actual_is_null() {
    assertThatExceptionOfType(AssertionError.class).isThrownBy(() -> maps.assertHasEntrySatisfying(INFO, null, 8, isDigits))
  ...
  void should_fail_if_actual_does_not_contain_key() {
    ...
    String key = "id";
    // WHEN
    expectAssertionError(() -> maps.assertHasEntrySatisfying(INFO, actual, key, isDigits));
    // THEN
  ...
  void should_fail_if_actual_contains_null_key_with_value_not_matching_condition() {
    ...
    String key = null;
    // WHEN
    expectAssertionError(() -> maps.assertHasEntrySatisfying(INFO, actual, key, nonNull));
    // THEN
  ...
  void should_pass_if_actual_contains_key_with_value_match_condition() {
    maps.assertHasEntrySatisfying(INFO, actual, "name", isNotDigits);
  }
}
