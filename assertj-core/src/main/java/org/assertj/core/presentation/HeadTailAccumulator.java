...
/**
 * Accumulates the values in a stream or iterable, keeping the first and last elements and
...
final class HeadTailAccumulator<T> {
  ...
  void add(final T element) {
    if (!head.offer(element)) {
      tail.offer(element);
    }
  }

  /**
   * Converts the accumulated elements into a list.
   *
